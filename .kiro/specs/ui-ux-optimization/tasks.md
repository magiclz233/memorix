...
- [x] 19.4 优化任务元数据计算
  ...
  - 实现 `recomputeTask` 增量更新逻辑，避免全量重新计算
  - 提升大量文件上传时的性能
  - _需求: NFR-4.1_
...
- [x] 27.2 优化媒体选择器对话框
  ...
  - 响应式适配（移动端全屏使用 Sheet，桌面端使用 Dialog）
  - 添加搜索和筛选功能
  - 优化加载状态与骨架屏
  - 添加批量快捷选择功能（全选/清空）
  - _需求: FR-6.1_
...
- [x] 28.3 优化存储状态指示器
  ...
  - 为“扫描中”状态添加蓝色脉冲呼吸灯动效
  - 优化各状态颜色的语义表达（Indigo/Amber/Rose）
  - 保持状态感知的一致性
  - _需求: FR-1.1_

- [x] 29. Checkpoint - 交互优化验证
  - 测试照片详情模态框的所有功能
  - 验证键盘导航和触摸手势
  - 测试前台筛选和无限滚动
  - 验证藏品管理的拖拽排序
...
- [x] 30.1 优化移动端触摸目标尺寸
  - 确保所有可点击元素（如刷新按钮、操作菜单）至少 44x44px
  - 在 `CollectionManager` 和 `MediaPicker` 中已实施
  - _需求: FR-6.1_

- [x] 30.2 优化移动端批量操作
  - 在 `MediaPicker` 中实现了全选/清空按钮
  - 在 `CollectionManager` 中适配了移动端 Sheet 操作
  - _需求: FR-6.1_

- [x] 30.3 优化移动端表格显示
  - 在 `StorageView` 中实现了响应式列表视图
  - _需求: FR-6.1_
...
- [x] 31.1 优化焦点样式
  - 为所有按钮和可交互组件添加了 `focus-visible` 样式
  - _需求: FR-7.1_

- [x] 31.2 优化 Tab 键顺序
  - 确保了 Dialog 和 Sheet 中的焦点流逻辑合理
  - _需求: FR-7.1_
...
- [x] 33. Checkpoint - 响应式与无障碍性验证
  - 在移动端、平板、桌面端完成适配测试
  - 验证 SSE 日志面板在大数据量下的自动滚动和交互
  - 校验 ARIA 标签在模态框和 Sheet 切换时的正确性
  - 确认所有新增文案的国际化同步
  - 询问用户是否有问题或需要调整
